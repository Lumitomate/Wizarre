...
func _process(_delta: float) -> bool:
	...
	if mine._levitate_landed and landed_step < 0:
		...
		ok("contenu visible: %.1f -> %.1f (bloc underside=%.1f, sol=%.1f)"
				% [content_top, content_bottom, block_underside_y, floor_y])
		if content_top <= block_underside_y:
			return fail("le haut de la mine visible (%.1f) touche/dépasse le bloc" % content_top)
